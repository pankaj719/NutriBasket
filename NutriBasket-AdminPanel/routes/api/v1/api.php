...
Route::group(['namespace' => 'Api\V1', 'middleware'=>'localization'], function () {
    ...
    Route::group(['prefix' => 'auth', 'namespace' => 'Auth'], function () {
        ...
        Route::group(['prefix' => 'packager'], function () {
            Route::post('login', [\App\Http\Controllers\Api\V1\Auth\B2BPackagerLoginController::class, 'login']);
            Route::middleware('auth:b2b_packagers')->group(function () {
                Route::get('profile', [\App\Http\Controllers\Api\V1\PackagerController::class, 'profile']);
                Route::get('orders', [\App\Http\Controllers\Api\V1\PackagerController::class, 'getOrders']);
                Route::get('order/{order_id}', [\App\Http\Controllers\Api\V1\PackagerController::class, 'getOrderDetails']);
                Route::get('orders/status/{status}', [\App\Http\Controllers\Api\V1\PackagerController::class, 'getOrdersByStatus']);
                Route::put('order/update-status', [\App\Http\Controllers\Api\V1\PackagerController::class, 'updateOrderStatus']);
                Route::get('orders/packaging', [\App\Http\Controllers\Api\V1\PackagerController::class, 'getPackagingOrders']);
                Route::get('orders/picked_up', [\App\Http\Controllers\Api\V1\PackagerController::class, 'getPickedUpOrders']);
            });
        });
    });
    ...
    Route::group(['middleware'=>['module-check']], function(){
        Route::group(['prefix' => 'customer', 'middleware' => 'auth:api'], function () {
            ...
            Route::group(['prefix' => 'templates'], function () {
                ...
                Route::post('/convert-from-order', 'TemplateController@convertFromOrder');
            });
            ...
            // Order Editing Routes (for all users with delivered orders) - Requires Authentication
            Route::group(['prefix' => 'order'], function () {
                // B2B Order Editing Routes (for delivered orders) - DEPRECATED, use generic routes below
                Route::get('b2b/editable/{order_id}', 'B2BOrderEditController@getEditableOrder');
                Route::post('b2b/submit-changes/{order_id}', 'B2BOrderEditController@submitQuantityChanges');
                Route::get('b2b/change-requests/{order_id}', 'B2BOrderEditController@getChangeRequestStatus');
                
                // Generic Order Editing Routes (for all users with delivered orders)
                Route::get('editable/{order_id}', 'B2BOrderEditController@getEditableOrder');
                Route::post('submit-changes/{order_id}', 'B2BOrderEditController@submitQuantityChanges');
                Route::get('change-requests/{order_id}', 'B2BOrderEditController@getChangeRequestStatus');
            });
...
Route::post('order-change-requests/{order}', [OrderChangeRequestController::class, 'store']);
Route::post('order-change-requests/{orderId}/bulk', [OrderChangeRequestController::class, 'bulkStore']);
