...
class Item extends Model
{
    ...
    /**
     * Calculate and update weighted average price for this item
     */
    public function calculateWeightedAveragePrice()
    {
        ...
        if ($purchases->isEmpty()) {
            $this->update(['weighted_average_price' => 0]);
            return 0;
        }
        ...
        foreach ($purchases as $purchase) {
            $totalValue += $purchase->price_per_unit * $purchase->quantity;
            $totalQuantity += $purchase->quantity;
        }

        $weightedAverage = $totalQuantity > 0 ? $totalValue / $totalQuantity : 0;
        
        $this->update(['weighted_average_price' => round($weightedAverage, 2)]);
