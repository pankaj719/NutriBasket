...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|in:cash_on_delivery,digital_payment,wallet,offline_payment,postpaid',
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        // Set delivery charge to 0 for postpaid - applied immediately after setting delivery charge
        if ($request->payment_method === 'postpaid') {
        ...
        ->get()->map(function ($data) {
            ...
            // Ensure quantity is preserved as decimal
        ...
        if ($request->order_type !== 'parcel') {
            foreach ($carts as $c) {
                // Ensure quantity is preserved as decimal when accessing as array
                $quantity = is_array($c) ? (float) $c['quantity'] : (float) $c->quantity;
                // Ensure price is preserved when accessing as array
                $base_price = is_array($c) ? (float) $c['price'] : (float) $c->price;
                // Ensure item_id is preserved when accessing as array
                $item_id = is_array($c) ? $c['item_id'] : $c->item_id;
                // Ensure item_type is preserved when accessing as array
                $item_type = is_array($c) ? $c['item_type'] : $c->item_type;
                // Ensure variation is preserved when accessing as array
                $variation = is_array($c) ? $c['variation'] : $c->variation;
                // Ensure add_on_ids is preserved when accessing as array
                $add_on_ids = is_array($c) ? $c['add_on_ids'] : $c->add_on_ids;
                // Ensure add_on_qtys is preserved when accessing as array
                $add_on_qtys = is_array($c) ? $c['add_on_qtys'] : $c->add_on_qtys;
                // Ensure variant is preserved when accessing as array
                $variant = is_array($c) ? $c['variant'] : $c->variant;
                
                if ($item_type === 'App\Models\ItemCampaign' || $item_type === 'AppModelsItemCampaign') {
                    $product = ItemCampaign::with('module')->active()->find($item_id);
                    
                    // Recalculate B2B pricing based on current user's contract status
                    // This ensures that managers who are not under the same contract get original pricing
                    // while users with contracts get their contract pricing
                    ...
                    if ($product) {
                        ...
                        // Maximum cart quantity validation removed - users can order any quantity
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            $product_variations = json_decode($product->food_variations, true);
                            $variations = [];
                            if (count($product_variations)) {
                                $variation_data = Helpers::get_varient($product_variations, $variation);
                                // Recalculate price based on current user's B2B status
                                $recalculated_base_price = $this->getB2BPriceForCurrentUser($product, $request->user);
                                $price = $recalculated_base_price + $variation_data['price'];
                                $variations =  $variation_data['variations'];
                            } else {
                                // Recalculate price based on current user's B2B status
                                $price = $this->getB2BPriceForCurrentUser($product, $request->user);
                            }
                            $product->tax = $store->tax;
                            $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
                            $addon_data = Helpers::calculate_addon_price(\App\Models\AddOn::whereIn('id', $add_on_ids)->get(), $add_on_qtys);
                            ...
                            if (count(json_decode($product['variations'], true)) > 0 && count($variation)>0) {
                                $variant_data = Helpers::variation_price($product, json_encode($variation));
                                // Recalculate base price based on current user's B2B status
                                $recalculated_base_price = $this->getB2BPriceForCurrentUser($product, $request->user);
                                $price = $recalculated_base_price + ($variant_data['price'] - $product->price);
                            ...
                            if (config('module.' . $product->module->module_type)['stock']) {
                                // Stock validation removed - users can order any quantity
                                $product_data[] = [
                                    'item' => clone $product,
                                    'quantity' => $quantity,
                                    'variant' => count($variation) > 0 ? $variation[0]['type'] : null
                                ];
                            }
                    ...
                    $product = Item::with('module')->active()->find($item_id);
        ...
        // Treat postpaid as cash_on_delivery everywhere
        $isCOD = in_array($request->payment_method, ['cash_on_delivery', 'postpaid']);
        if (isset($module_wise_delivery_charge) && $isCOD && $module_wise_delivery_charge->pivot->maximum_cod_order_amount && $order->order_amount > $module_wise_delivery_charge->pivot->maximum_cod_order_amount) {
            return response()->json([
                'errors' => [
        ...
        try {
            ...
            if ($request->order_type !== 'parcel') {
                ...
                // Group order details by item_id and variation before inserting
                $grouped_order_details = $this->groupOrderDetailsForDatabase($order_details);
                
                foreach ($grouped_order_details as $key => $item) {
                    $grouped_order_details[$key]['order_id'] = $order->id;

                    if($store_discount_amount <= 0 ){
                        $grouped_order_details[$key]['discount_on_item'] = 0;
                    }
                }
                OrderDetail::insert($grouped_order_details);
                // REMOVED: Stock deduction during order placement
                // Stock will only be deducted when order moves to processing/accepted stage
...
$store = Store::with(['discount', 'store_sub'])
    ...
    ->first();
        ...
        if(isset($cod['status']) &&  $cod['status'] != 1 && $store->zone->cash_on_delivery!=1 && $isCOD){
            return response()->json([
                'errors' => [
        ...
        // Set delivery charge to 0 for postpaid - will be applied after all calculations
...
//        })
        ...
        if ($details != null && $details->count() > 0) {
            ...
            // Add modification information to the response
            if ($order->quantity_modified) {
                $details[0]['order_modified'] = true;
                $details[0]['modification_reason'] = $order->modification_reason;
                $details[0]['modified_at'] = $order->modified_at;
            } else {
                $details[0]['order_modified'] = false;
            }
    ...
    private function createCashBackHistory($order_amount, $user_id,$order_id){
        ...
        if($user && $user->loyalty_point > 0){
            $point = $user->loyalty_point;
            $user->loyalty_point = 0;
            $user->save();
            $loyalty_point_transaction = new LoyaltyPointTransaction();
            $loyalty_point_transaction->user_id = $user_id;
            $loyalty_point_transaction->order_id = $order_id;
            $loyalty_point_transaction->point = $point;
            $loyalty_point_transaction->is_credit = 0;
    ...
    /**
     * Get B2B price for the current user based on their contract status
     * This method ensures that managers who are not under the same contract get original pricing
     * while users with contracts get their contract pricing
     */
    private function getB2BPriceForCurrentUser($product, $user)
    {
        if ($user && $user->user_type === 'b2b') {
            $contract = $user->getActiveClientContract();
            if ($contract) {
                $contractItem = \App\Models\ContractItem::where('contract_id', $contract->id)
                    ->where('item_id', $product->id)
                    ->first();
                if ($contractItem && $contractItem->price !== null) {
                    return $contractItem->price;
                }
            }
            // If no contract item found, check for b2b_price field
            if (isset($product->b2b_price)) {
    ...
    /**
     * Group order details by item_id and variation for database insertion
     * This consolidates same items into single entries with summed quantities
     */
    private function groupOrderDetailsForDatabase($order_details)
    {
        $grouped = [];
        
        foreach ($order_details as $detail) {
            // Create a unique key for each item based on item_id and variation
            $itemKey = $detail['item_id'];
            if (isset($detail['variation']) && $detail['variation']) {
                $variation = json_decode($detail['variation'], true);
                if ($variation) {
                    $itemKey .= '_' . md5(json_encode($variation));
                }
            }
            ...
            if (!isset($grouped[$itemKey])) {
                $grouped[$itemKey] = [
                    ...
                    'item_details' => $detail['item_details'],
                    ...
                    'total_add_on_price' => $detail['total_add_on_price'],
                    'quantity' => 0,
            ...
            $grouped[$itemKey]['quantity'] += $detail['quantity'];
            
            // Recalculate tax amount based on new total quantity
            $original_price = $detail['price'];
            $original_quantity = $detail['quantity'];
            $new_quantity = $grouped[$itemKey]['quantity'];
            
            // Update tax amount proportionally
            $grouped[$itemKey]['tax_amount'] = round(($detail['tax_amount'] / $original_quantity) * $new_quantity, config('round_up_to_digit'));
            
            // Update discount amount proportionally
            $grouped[$itemKey]['discount_on_item'] = round(($detail['discount_on_item'] / $original_quantity) * $new_quantity, config('round_up_to_digit'));
            
            // Update add-on price proportionally
            $grouped[$itemKey]['total_add_on_price'] = round(($detail['total_add_on_price'] / $original_quantity) * $new_quantity, config('round_up_to_digit'));
        }
