...
class ItemController extends Controller
{
    ...
    public function get_new_products(Request $request)
    {
        ...
        // B2B contract pricing logic
        if (auth('api')->check() && auth('api')->user()->user_type === 'b2b') {
            $user = auth('api')->user();
    ...
    public function get_product($id)
    {
        try {
            ...
            // B2B contract pricing logic for single product
            if (auth('api')->check() && auth('api')->user()->user_type === 'b2b') {
                $user = auth('api')->user();
                $contract = $user->getActiveClientContract();
                if ($contract) {
...
// if (auth('api')->check() && auth('api')->user()->user_type === 'b2b') {
//     $user = auth('api')->user();
//     $contract = \App\Models\Contract::where('vendor_id', $user->id)->where('status', 'active')->first();
//     if ($contract) {
//         foreach ($items['products'] as $item) {
//             $contractItem = \App\Models\ContractItem::where('contract_id', $contract->id)
//                 ->where('item_id', $item->id)
