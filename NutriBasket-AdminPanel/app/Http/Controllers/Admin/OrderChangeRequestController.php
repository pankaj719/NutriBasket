<?php
// app/Http/Controllers/Admin/OrderChangeRequestController.php
...
class OrderChangeRequestController extends Controller
{
    ...
    public function approve($id)
    {
        ...
        $orderItem = OrderDetail::where('id', $request->item_id)->first();
        if ($orderItem) {
            $orderItem->quantity = $request->new_quantity;
            $orderItem->save();
        }

        // Get the order to check if it's delivered
        $order = $request->order;
    ...
    {
        $requests = OrderChangeRequest::where('order_id', $orderId)->where('status', 'pending')->get();
        foreach ($requests as $request) {
            $request->status = 'approved';
            $request->save();

            $orderItem = \App\Models\OrderDetail::where('id', $request->item_id)->first();
            if ($orderItem) {
                $orderItem->quantity = $request->new_quantity;
                $orderItem->save();
            }
        }
        return redirect()->back()->with('success', 'All pending change requests for this order approved.');
    }
    ...
    public function rejectOrder($orderId)
    {
        ...
        return redirect()->back()->with('success', 'All pending change requests for this order declined.');
    }
}