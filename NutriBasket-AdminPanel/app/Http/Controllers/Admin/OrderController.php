...
use App\Models\OrderChangeRequest; // Make sure to import your model
...
class OrderController extends Controller
{
    public function list($status, Request $request)
    {
        ...
        $total = $orders->total();
            ...
            $total_items = [];
    $customer_items = [];
    $pending_order_type = $request->pending_order_type ?? 'total';

if ($status == 'pending' && $pending_order_type == 'total') {
...
$stock_diff_items = [];
if ($status == 'pending' && $pending_order_type == 'stock_difference') {
    ...
    if ($status == 'pending' && $pending_order_type == 'by_customer') {
        $customer_items_raw = \DB::table('order_details')
            ->join('orders', 'order_details.order_id', '=', 'orders.id')
            ->join('items', 'order_details.item_id', '=', 'items.id')
            ...
            ->select(
                ...
                \DB::raw("CONCAT(users.f_name, ' ', users.l_name) as customer_name"),
                'items.name as item_name',
                \DB::raw('SUM(order_details.quantity) as quantity')
            )
            ->groupBy('users.id', 'users.f_name', 'users.l_name', 'items.id', 'items.name')
            ->get();

        $customer_items = [];
        foreach ($customer_items_raw as $row) {
            $customer_items[$row->customer_id]['customer_id'] = $row->customer_id;
            $customer_items[$row->customer_id]['customer_name'] = $row->customer_name;
            $customer_items[$row->customer_id]['items'][] = [
                'item_name' => $row->item_name,
                'quantity' => $row->quantity,
            ];
        }
        $customer_items = array_values($customer_items);
    }

    // ...existing code...
    return view('admin-views.order.list', compact(
        'orders', 'status', 'orderstatus', 'scheduled', 'vendor_ids', 'zone_ids', 'from_date', 'to_date', 'total', 'order_type',
        'total_items', 'customer_items', 'pending_order_type',    'stock_diff_items'
    ));
 }
 
//stockdifference
public function stockDifferenceReport()
{
    // Get total ordered quantity per item for pending orders
    $ordered = \DB::table('order_details')
        ->join('orders', 'order_details.order_id', '=', 'orders.id')
        ->where('orders.order_status', 'pending')
        ->select('order_details.item_id', \DB::raw('SUM(order_details.quantity) as total_ordered'))
    ...
    return view('admin-views.order.stock-difference', compact('report'));
}
// StockDifferenceExopt
public function stockDifferenceExport($type)
{
    // Same logic as your stockDifferenceReport to build $report
    $ordered = \DB::table('order_details')
        ->join('orders', 'order_details.order_id', '=', 'orders.id')
        ->where('orders.order_status', 'pending')
        ->select('order_details.item_id', \DB::raw('SUM(order_details.quantity) as total_ordered'))
...
foreach ($items as $item) {
    ...
    if ($type === 'csv') {
        // CSV export
        $filename = 'stock_difference_report.csv';
        ...
        return Excel::download(new \App\Exports\ArrayExport($report), 'stock_difference_report.xlsx');
    }
}
...
public function updateItemStock(Request $request)
{
    ...
    $item->save();
    \Brian2694\Toastr\Facades\Toastr::success('Stock updated!');
    return back();
}
//Process all orders in one go
public function processAllOrders(Request $request)
{
    // Get all pending orders
    $orders = \App\Models\Order::with('details.item')
    ...
    foreach ($orders as $order) {
        ...
        $order->processing_time = now()->timestamp;
        $order->save();
    }

    \Brian2694\Toastr\Facades\Toastr::success('All pending orders processed and stock updated!');
    return back();
}
...
//process by customer 
public function processByCustomer(Request $request)
{
    ...
    \Brian2694\Toastr\Facades\Toastr::success('Orders processed and stock updated!');
    return back();
}
    ...
    public function details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            ...
            // Group order details by item
            $groupedDetails = $this->groupOrderDetails($order->details);

            $deliveryMen = Helpers::deliverymen_list_formatting($deliveryMen);
            return view('admin-views.order.order-view', compact('order', 'deliveryMen', 'categories', 'products', 'category', 'keyword', 'editing', 'groupedDetails'));
        } else {
    ...
    /**
     * Group order details by item to consolidate same items
     */
    private function groupOrderDetails($details)
    {
        ...
        foreach ($details as $detail) {
            if (isset($detail->item_id)) {
                ...
                $grouped[$itemKey]->total_amount += ($detail->price * $detail->quantity);
                $grouped[$itemKey]->total_discount += ($detail->discount_on_item * $detail->quantity);
                $grouped[$itemKey]->total_tax += ($detail->tax_amount * $detail->quantity);
                $grouped[$itemKey]->original_details[] = $detail;
            } elseif (isset($detail->item_campaign_id)) {
                // Handle campaign items separately
                $campaignKey = 'campaign_' . $detail->item_campaign_id;
                if (!isset($grouped[$campaignKey])) {
                    $grouped[$campaignKey] = (object) [
                        'item_id' => null,
                        'item_campaign_id' => $detail->item_campaign_id,
                        'item_details' => $detail->item_details,
                        'variation' => $detail->variation,
                ...
                $grouped[$campaignKey]->quantity += $detail->quantity;
                $grouped[$campaignKey]->total_amount += ($detail->price * $detail->quantity);
                $grouped[$campaignKey]->total_discount += ($detail->discount_on_item * $detail->quantity);
                $grouped[$campaignKey]->total_tax += ($detail->tax_amount * $detail->quantity);
                $grouped[$campaignKey]->original_details[] = $detail;
            }
        }
    ...
    public function all_details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            ...
            // Apply grouping to order details
            $groupedDetails = $this->groupOrderDetails($order->details);
    ...
    public function status(Request $request)
    {
        ...
        // Deduct stock when order moves from 'preparing' to 'accepted' status
        if($request->order_status == 'accepted' && $order->order_status == 'preparing') {
            if (config('module.' . $order->module->module_type)['stock']) {
                foreach ($order->details as $detail) {
                    $variant = json_decode($detail['variation'], true);
                    $item = $detail->item;
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if (isset($product->module_id) && $product->module->module_type == 'food' && $product->food_variations) {
            ...
            $price = $this->getB2BPrice($product, null) + $variation_price;
        ...
        if (auth()->check() && auth()->user()->user_type === 'b2b') {
            $user = auth()->user();
    ...
    public function update(Request $request, Order $order)
    {
                ...
                if ($c['status'] == true) {
                    ...
                    if ($c['item_campaign_id'] != null) {
                        ...
                        if ($product) {
                           ...
                           $price = $this->getB2BPrice($product, null);
      ...
      //Added service charge
        ...
        // Set delivery charge to 0 for postpaid - applied after all calculations
        if ($order->payment_method === 'postpaid') {
            $order->delivery_charge = 0;
            $order->original_delivery_charge = 0;
            // Recalculate order_amount without delivery charge
            $order->order_amount = $total_price + $total_tax_amount + $order->additional_charge;
        }
    ...
    public function offline_payment(Request $request){
            ...
            else if($request->verify == 'switched_to_cod'){
...
public function requestQuantityChange(Request $request, $orderId)
{
    $request->validate([
...
private function getB2BPrice($product, $user_id)
{
    // Use current authenticated user instead of user_id parameter
