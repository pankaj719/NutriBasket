...
class Helpers
{
    ...
    public static function order_details_data_formatting($data)
    {
        ...
        foreach ($data as $item) {
            ...
            // Handle NA items - if item is marked as NA, set quantity to "NA" and price to 0
            if (isset($item['is_na']) && $item['is_na']) {
                $item['quantity'] = 'NA';
                $item['price'] = 0;
                $item['total_price'] = 0;
            }
    ...
    public static function sendNotificationToHttp(array|null $data)
    {
        ...
        if(data_get($key,'project_id')){
            ...
            try {
                ...
                \Log::error('Firebase notification exception: ' . $exception->getMessage());
                return false;
            }
        }
        return false;
...
//        }
        ...
        try {
            ...
            if((in_array($order->payment_method, ['cash_on_delivery', 'offline_payment'])  && $order->order_status == 'pending' )||(!in_array($order->payment_method, ['cash_on_delivery', 'offline_payment']) && $order->order_status == 'confirmed' )){
                ...
                if (!$order->is_guest && $order->customer) {
                    $is_b2b = ($order->customer->user_type === 'b2b');
                }
            ...
            if (self::getNotificationStatusData('customer','customer_order_notification','push_notification_status') &&  $value && $user_fcm) {
                ...
                $notification_sent = self::send_push_notif_to_device($user_fcm, $data);
                if ($notification_sent) {
                    ...
                    \Log::error('Failed to send push notification to user for order #' . $order->id);
                }
            } else {
                \Log::warning('Notification conditions not met for order #' . $order->id . ': notification_enabled=' . (self::getNotificationStatusData('customer','customer_order_notification','push_notification_status') ? 'true' : 'false') . ', value=' . ($value ? 'set' : 'empty') . ', fcm_token=' . ($user_fcm ? 'set' : 'empty'));
            }
    ...
    public static function get_language_name($key)
    {
        $languages = array(
            ...
            "ca" => "Catalan - català",
            "ckb" => "Central Kurdish - کوردی (دەستنوی عەرەبی)",
            "zh" => "Chinese - 中文",
            "zh-HK" => "Chinese (Hong Kong) - 中文（香港）",
            "zh-CN" => "Chinese (Simplified) - 中文（体）",
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
        ...
        if ($data) {
            // For production servers that don't properly serve symbolic links
            if (app()->environment('production') || request()->getHost() === 'backend.nutribasket.in') {
