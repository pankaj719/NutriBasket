...
$(document).on("click", ".decrease-button", function () {
    ...
    let currentValue = parseFloat(addon_quantity_input.val());
    if (currentValue > 0.1) {
        addon_quantity_input.val((currentValue - 0.1).toFixed(1));
        getVariantPrice();
    }
});
...
$(document).on("click", ".increase-button-cart", function () {
    let addon_quantity_input = $('input[name="quantity"]');
    let currentValue = parseFloat(addon_quantity_input.val());
    let maxValue = parseFloat(addon_quantity_input.attr("max"));
    if (maxValue - 0.1 >= currentValue) {
        addon_quantity_input.val((currentValue + 0.1).toFixed(1));
        getVariantPrice();
    } else {
        Swal.fire({
