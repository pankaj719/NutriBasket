                                            ...
                                            @php
                                                ...
                                                $b2b_price = $user && $user->user_type === 'b2b'
                                                    ? optional(\App\Models\ContractItem::where('contract_id', optional(\App\Models\Contract::where('client_id', $user->id)->where('status', 'active')->first())->id)
                                                        ->where('item_id', $transaction->package_id)->first())->price
                                                    : null;
                                                $unit_price = $b2b_price ?? $transaction->paid_amount;
                                            @endphp
                                            <span class="__txt-nowrap">
                                                {{  App\CentralLogics\Helpers::format_currency($unit_price) }}
                                            </span>
                                        </td>
                                    </tr>
                                </tbody>
