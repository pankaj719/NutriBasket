                    ...
                    <div class="card-body px-0">
                        ...
                        @else
                                ...
                                if ($editing) {
                                    ...
                                    // Use grouped details if available, otherwise use original details
                                    if (isset($groupedDetails) && !empty($groupedDetails)) {
                            ...
                            <div class="table-responsive">
                                <table
                                    ...
                                    @foreach ($details as $key => $detail)
                                        @if ((isset($detail->item_id) || isset($detail['item_id'])) && (isset($detail->status) ? $detail->status : $detail['status']))
                                                <?php
                                                if (!$editing) {
                                                    if (is_object($detail)) {
                                                ...
                                                $itemId = is_object($detail) ? $detail->item_id : $detail['item_id'];
                                                $itemDetails = is_object($detail) ? $detail->item_details : $detail['item_details'];
                                                
                                                $product = \App\Models\Item::where(['id' => data_get($detail['item'] ?? $detail->item,'id')])->first();
                                                if(!$product){
                                                    if (is_object($detail)) {
                                                        $detail->item = json_decode($detail->item_details, true);
                                                    } else {
                                            ...
                                            <tr>
                                                    ...
                                                    <div class="media media--sm">
                                                        ...
                                                        @else
                                                            <a class="avatar avatar-xl mr-3"
                                                               href="{{ route('admin.item.view', [is_object($detail) ? $detail->item['id'] : $detail['item']['id'],'module_id' => $order->module_id]) }}">
                                                                <img class="img-fluid rounded aspect-ratio-1 onerror-image"
                                                                     src="{{ $product?->image_full_url ?? asset('public/assets/admin/img/100x100/2.png') }}"
                                                                     data-onerror-image="{{ asset('public/assets/admin/img/100x100/2.png') }}"
                                                                     alt="Image Description">
                                                        ...
                                                        <div class="media-body">
                                                            <div>
                                                                <strong class="line--limit-1">
                                                                    {{ is_object($detail) ? $detail->item['name'] : $detail['item']['name'] }}</strong>
                                                                <h6>
                                                                    {{ is_object($detail) ? $detail->quantity : $detail['quantity'] }} x
                                                                    {{ \App\CentralLogics\Helpers::format_currency(is_object($detail) ? $detail->price : $detail['price']) }}
                                                                </h6>
                                                                @if ($order->store && $order->store->module->module_type == 'food')
                                                                    @if (isset(is_object($detail) ? $detail->variation : $detail['variation']) ? json_decode(is_object($detail) ? $detail->variation : $detail['variation'], true) : [])
                                                                        @foreach (json_decode(is_object($detail) ? $detail->variation : $detail['variation'], true) as $variation)
                                                                            @if (isset($variation['name']) && isset($variation['values']))
                                                                                ...
                                                                                @endforeach
                                                                            @else
                                                                                @if (isset(json_decode(is_object($detail) ? $detail->variation : $detail['variation'], true)[0]))
                                                                                    <strong><u>
                                                                                            {{ translate('messages.Variation') }}
                                                                                            : </u></strong>
                                                                                    @foreach (json_decode(is_object($detail) ? $detail->variation : $detail['variation'], true)[0] as $key1 => $variation)
                                                                                        <div
                                                ...
                                                @if ($order->store && $order->store->module->module_type == 'food')
                                                    <td>
                                                        <div>
                                                            @foreach (json_decode(is_object($detail) ? $detail->add_ons : $detail['add_ons'], true) as $key2 => $addon)
                                                                @if ($key2 == 0)
                                                ...
                                                <td class="text-right">
                                                    <div>
                                                        @php($amount = (is_object($detail) ? $detail->price : $detail['price']) * (is_object($detail) ? $detail->quantity : $detail['quantity']))
    ...
    <script>
        ...
        function cartQuantityInitialize() {
                ...
                if (!isNaN(currentVal)) {
                    if (type == 'minus') {
                        ...
                        if (currentVal > parseFloat(input.attr('min'))) {
                            input.val((currentVal - 0.1).toFixed(1)).change();
                        }
            ...
            $('.input-number').change(function() {
                ...
                // Stock limit validation removed - users can order any quantity
