...
<tr class="status-{{$order['order_status']}} class-all">
    ...
    <td class="text-capitalize text-center">
        ...
        @elseif($order['order_status']=='delivered')
            ...
            @if($order->customer && $order->customer->user_type === 'b2b' && $order->delivered)
                @php
                    $deliveredAt = \Carbon\Carbon::parse($order->delivered);
                    $twentyFourHoursLater = $deliveredAt->addHours(24);
                    $now = \Carbon\Carbon::now();
                    $timeRemaining = $now->diffInHours($twentyFourHoursLater, false);
                @endphp
                @if($timeRemaining > 0)
                    <div class="text-warning mt-1 small">
                        <i class="tio-clock"></i> {{translate('messages.auto_acknowledge_in')}} {{ ceil($timeRemaining) }}h
                    </div>
                @else
                    <div class="text-info mt-1 small">
                        <i class="tio-checkmark"></i> {{translate('messages.ready_for_acknowledgment')}}
                    </div>
                @endif
            @endif
