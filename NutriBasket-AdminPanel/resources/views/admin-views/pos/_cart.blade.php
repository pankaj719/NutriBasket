        ...
        @if(session()->has('cart') && count( session()->get('cart')) > 0)
            ...
            <tr>
                ...
                <td class="text-center middle-align">
                    <input type="number"  data-key="{{$key}}"  class="amount--input form-control text-center update-Quantity" data-oldvalue="{{$cartItem['quantity']}}" value="{{$cartItem['quantity']}}" min="0.1"
                    max="{{ (isset($cartItem['stock_quantity']) && $cartItem['stock_quantity'] > 0) ?   ($cartItem['maximum_cart_quantity'] ?  min($cartItem['stock_quantity'], $cartItem['maximum_cart_quantity']) : $cartItem['stock_quantity'])  : $cartItem['maximum_cart_quantity'] ??  '9999999999' }}" step="0.1" >
                </td>
                @php
                    $user = session('customer') ? \App\Models\User::find(session('customer')->id) : null;
                    $b2b_price = $user && $user->user_type === 'b2b'
                        ? optional(\App\Models\ContractItem::where('contract_id', optional(\App\Models\Contract::where('client_id', $user->id)->where('status', 'active')->first())->id)
                            ->where('item_id', $cartItem['id'])->first())->price
                        : null;
                    $unit_price = $b2b_price ?? $cartItem['price'];
                    $product_subtotal = $unit_price * $cartItem['quantity'];
                @endphp
