...
class OrderRepository implements OrderRepositoryInterface {
  ...
  Future<Response> submitOrderChangeRequest(
    ...
    return await apiClient.postData(
        '${AppConstants.orderChangeRequestUri}/$orderId', data);
  }
  ...
  Future<Response> submitBulkOrderChangeRequest(
    ...
    return await apiClient.postData(
        '${AppConstants.orderChangeRequestBulkUri}/$orderId/bulk', data);
  }

  Future<Response> getB2bEditableOrder(String orderId,
      {String? guestId}) async {
    return await apiClient.getData(
      '${AppConstants.b2bEditableOrderUri}/$orderId${guestId != null ? '?guest_id=$guestId' : ''}',
    );
  }
  ...
  Future<Response> submitB2bOrderChanges(
      ...
      {String? guestId}) async {
    Map<String, dynamic> data = Map.from(requestBody);
    if (AuthHelper.isGuestLoggedIn() || guestId != null) {
      data.addAll({'guest_id': guestId ?? AuthHelper.getGuestId()});
    }
    return await apiClient.postData(
        '${AppConstants.b2bSubmitChangesUri}/$orderId', data);
  }

  Future<Response> getB2bChangeRequests(String orderId,
      {String? guestId}) async {
    return await apiClient.getData(
      '${AppConstants.b2bChangeRequestsUri}/$orderId${guestId != null ? '?guest_id=$guestId' : ''}',
    );
  }
}
