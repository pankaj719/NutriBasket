...
class ProfileController extends GetxController implements GetxService {
  ...
  Future<void> getUserInfo() async {
    ...
    if (userInfoModel != null) {
      ...
      // If user is B2B and has contract pricing, notify item controller to refresh
      if (userInfoModel.isB2BUser == true && userInfoModel.b2bContractPricing != null) {
        try {
          // Force item controller to update/refresh items with new B2B prices
          if (Get.isRegistered<ItemController>()) {
            Get.find<ItemController>().update();
          }
        } catch (e) {
          // ItemController might not be registered yet, that's okay
        }
      }
    }
    update();
