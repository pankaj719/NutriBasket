...
abstract class CartServiceInterface {
  ...
  ModuleModel? forcefullySetModule(
      ModuleModel? module, List<ModuleModel>? moduleList, int moduleId);
  ...
  Future<double> calculateDiscountedPrice(
      CartModel cartModel, double quantity, bool isFoodVariation);
  Future<bool> updateCartQuantityOnline(
      int cartId, double price, double quantity);
  ...
  Future<List<OnlineCartModel>?> updateCartOnline(OnlineCartModel cart);
  Future<List<OnlineCartModel>?> addToCartOnline(OnlineCartModel cart);
