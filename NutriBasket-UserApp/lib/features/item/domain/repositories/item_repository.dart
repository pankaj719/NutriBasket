...
class ItemRepository implements ItemRepositoryInterface {
  ...
  Future getList(
    ...
    if (isPopularItem) {
      ...
      return await _getDiscountedItemList(type!,
          source: source ?? DataSourceEnum.client);
    } else if (isLatestItem) {
      return await _getLatestItemList(type!,
          source: source ?? DataSourceEnum.client);
    }
  }
  ...
  Future<List<Item>?> _getLatestItemList(String type,
      {required DataSourceEnum source}) async {
    List<Item>? latestItemList;
    String cacheId =
        '${AppConstants.newArrivalItemsUri}?limit=50&offset=1${Get.find<SplashController>().module!.id!}';

    switch (source) {
      case DataSourceEnum.client:
        // Use new arrival endpoint to get latest items
        String url = '${AppConstants.newArrivalItemsUri}?limit=50&offset=1';
        print('Fetching latest items from: $url');
        Response response = await apiClient.getData(url);
        print('Latest items response status: ${response.statusCode}');
        if (response.statusCode == 200) {
          latestItemList = [];
          latestItemList.addAll(ItemModel.fromJson(response.body).items!);
          print('Latest items fetched: ${latestItemList.length} items');
          LocalClient.organize(DataSourceEnum.client, cacheId,
              jsonEncode(response.body), apiClient.getHeader());
        } else {
          print(
              'Latest items API error: ${response.statusCode} - ${response.body}');
        }
      ...
      case DataSourceEnum.local:
        ...
        if (cacheResponseData != null) {
          ...
          latestItemList
              .addAll(ItemModel.fromJson(jsonDecode(cacheResponseData)).items!);
          print('Latest items from cache: ${latestItemList.length} items');
        }
    }
