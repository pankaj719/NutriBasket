...
class ItemService implements ItemServiceInterface {
  ...
  @override
  Future<List<Item>?> getLatestItemList(
      String type, DataSourceEnum? source) async {
    return await itemRepositoryInterface.getList(
        type: type, isLatestItem: true, source: source);
  }
  ...
  List<double?> initializeCartAddonsQtyList(
      List<AddOn>? addOnIds, List<AddOns>? addOns) {
    List<double?> addOnIdList = [];
    List<double?> addOnQtyList = [];
    for (var addOnId in addOnIds!) {
      addOnIdList.add(addOnId.id?.toDouble());
    }
    for (var addOn in addOns!) {
      if (addOnIdList.contains(addOn.id?.toDouble())) {
        addOnQtyList
            .add(addOnIds[addOnIdList.indexOf(addOn.id?.toDouble())].quantity);
      } else {
        addOnQtyList.add(1.0);
      }
    }
    return addOnQtyList;
  }
  ...
  double? getStartingPrice(Item item) {
    ...
    // Apply B2B pricing if available for this item
    if (B2BPricingHelper.shouldUseB2BPricing() && item.id != null) {
      final b2bPrice = B2BPricingHelper.getB2BContractPrice(item.id!);
      if (b2bPrice != null) {
        if (kDebugMode) {
          print('ItemService.getStartingPrice: Item ${item.id} (${item.name}) - Original: $startingPrice, B2B: $b2bPrice');
        }
        startingPrice = b2bPrice;
      }
    }
