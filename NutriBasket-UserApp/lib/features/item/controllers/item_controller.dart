...
class ItemController extends GetxController implements GetxService {
  ...
  List<double?> _addOnQtyList = [];
  List<double?> get addOnQtyList => _addOnQtyList;
  ...
  Future<void> getLatestItemList(bool reload, String type, bool notify,
      {DataSourceEnum dataSource = DataSourceEnum.local,
      bool fromRecall = false}) async {
    _latestType = type;
    if (reload) {
    ...
    if (_latestItemList == null || reload || fromRecall) {
      ...
      if (dataSource == DataSourceEnum.local) {
        ...
        getLatestItemList(false, type, notify,
            dataSource: DataSourceEnum.client, fromRecall: true);
      } else {
        items = await itemServiceInterface.getLatestItemList(type, dataSource);
        _prepareLatestItems(items);
  ...
  _prepareLatestItems(List<Item>? items) {
    if (items != null) {
      ...
      _isLoading = false;
      print('Latest items loaded: ${items.length} items');
    } else {
      print('Latest items is null');
    }
    update();
  }
  ...
  void setAddOnQuantity(bool isIncrement, int index) {
    _addOnQtyList[index] = itemServiceInterface.setAddOnQuantity(
        isIncrement, _addOnQtyList[index] ?? 1.0);
    update();
  }
  ...
  void setManualQuantity(double quantity) {
    if (quantity > 0) {
      _quantity = quantity;
      update();
    }
  ...
  void itemDirectlyAddToCart(Item? item, BuildContext context,
    ...
    if (((item!.foodVariations != null && item.foodVariations!.isEmpty) &&
      ...
      // Always allow adding to cart regardless of stock
      Get.find<CartController>().addToCartOnline(cartModel);
      showCartSnackBar();
