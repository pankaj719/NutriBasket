...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  TextEditingController guestContactPersonNameController =
      TextEditingController();
  TextEditingController guestContactPersonNumberController =
      TextEditingController();
  ...
  TextEditingController guestConfirmPasswordController =
      TextEditingController();
  ...
  Future<void> initCall() async {
    ...
    if (widget.storeId == null) {
      // Initialize cart data if needed
      if (GetPlatform.isWeb) {
        await Get.find<CartController>().getCartDataOnline();
      }

      // Set up checkout data if cart has items
        ...
        child: Scaffold(
          ...
          body: GetBuilder<CheckoutController>(builder: (checkoutController) {
            ...
            if (Get.find<OrderController>().trackModel != null) {
              total = ((Get.find<OrderController>().trackModel!.orderAmount! /
                      100) *
                  Get.find<SplashController>()
                      .configModel!
                      .loyaltyPointItemPurchasePoint!);
              success = Get.find<OrderController>().trackModel!.paymentStatus ==
                      'paid' ||
                  Get.find<OrderController>().trackModel!.paymentMethod ==
                      'cash_on_delivery' ||
                  Get.find<OrderController>().trackModel!.paymentMethod ==
                      'partial_payment' ||
                  Get.find<OrderController>().trackModel!.paymentMethod ==
                      'postpaid';
              parcel = Get.find<OrderController>().trackModel!.paymentMethod ==
                  'parcel';
              for (ZoneData zData
                ...
                Future.delayed(const Duration(seconds: 1), () {
                  Get.dialog(
                      PaymentFailedDialog(
                        ...
                        guestId: widget.storeId?.toString() ?? '',
                      ),
              ...
              if (success &&
                ...
                RepeatOrderHelper.storeLastOrder(
                    Get.find<OrderController>().trackModel!.id.toString(),
            ...
            return GetBuilder<CouponController>(builder: (couponController) {
              ...
              // Use current cart list for all calculations
              List<CartModel> currentCartList =
                  Get.find<CartController>().cartList;

              double price = _calculatePrice(
                  store: checkoutController.store, cartList: currentCartList);
              ...
              // Set delivery charge to 0 if Postpaid is selected
              if (Get.find<CheckoutController>().paymentMethodIndex == 2 &&
                  Get.find<CheckoutController>()
                          .digitalPaymentName
                          ?.toLowerCase() ==
                      'postpaid') {
            ...
            onPressed: checkoutController.acceptTerms
                ? () async {
                    ...
                    if (checkoutController.timeSlots == null ||
                      ...
                      // Use current cart list for availability check
                      List<CartModel> currentCartList =
                          Get.find<CartController>().cartList;
                      if (currentCartList.isNotEmpty) {
                        for (CartModel cart in currentCartList) {
                    ...
                    if (isGuestLogIn &&
                      ...
                      // Store hours validation bypassed - always allow ordering
                      ...
                      // Time slot validation bypassed - always allow ordering
                      ...
                      if (widget.storeId == null) {
                        // Use the current cart list from cart controller instead of static _cartList
                        List<CartModel> currentCartList =
                            Get.find<CartController>().cartList;

                        // Debug: Print cart state before order placement
                        print('Cart state before order placement:');
                        print('Cart items count: ${currentCartList.length}');
                        for (int i = 0; i < currentCartList.length; i++) {
                          print(
                              '  Item ${i + 1}: ${currentCartList[i].item?.name} - Qty: ${currentCartList[i].quantity}');
                        }

                        List<OnlineCart> carts = [];
                        for (int index = 0;
                            index < currentCartList.length;
                            index++) {
                          CartModel cart = currentCartList[index];
                          print(
                              'Converting cart item ${index + 1}: ${cart.item?.name}');

                          List<int?> addOnIdList = [];
                          List<double> addOnQtyList = [];
                          for (var addOn in cart.addOnIds!) {
                            addOnIdList.add(addOn.id);
                            addOnQtyList.add(addOn.quantity ?? 0.0);
                          }
                          ...
                          carts.add(OnlineCart(
                            cart.id ??
                                0, // Use 0 for template items that don't have cart IDs
                            cart.item!.id,
                          ...
                          print('Created OnlineCart for: ${cart.item?.name}');
                        }

                        print(
                            'Total OnlineCart items created: ${carts.length}');

                        // If postpaid, exclude delivery charge from order_amount
                        double orderAmountForBackend = total;
                        if ((checkoutController.paymentMethodIndex == 2 &&
                            checkoutController.digitalPaymentName
                                    ?.toLowerCase() ==
                                'postpaid')) {
                          // deliveryCharge is set to 0 for postpaid, but to be robust, subtract the original delivery charge if any
                          double deliveryChargeForCalc =
                          ...
                          orderAmountForBackend = total - deliveryChargeForCalc;
                        }

                        // Ensure delivery charge is 0 for postpaid payment method
                        double finalDeliveryCharge = deliveryCharge ?? 0;
                        if (checkoutController.paymentMethodIndex == 2 &&
                            checkoutController.digitalPaymentName
                                    ?.toLowerCase() ==
                                'postpaid') {
                        ...
                        PlaceOrderBodyModel placeOrderBody =
                          ...
                          scheduleAt: !checkoutController.store!.scheduleOrder!
                              ...
                              : (checkoutController.selectedDateSlot == 0 &&
                                  ...
                                  : DateConverter.dateToDateAndTime(
                                      scheduleEndDate),
                          orderAmount: orderAmountForBackend,
                          ...
                          couponCode:
                              (Get.find<CouponController>().discount! > 0 ||
                                  ...
                                  : null,
                          storeId: currentCartList.isNotEmpty
                              ? currentCartList[0].item!.storeId
                              : null,
                        ...
                        print(
                            'PlaceOrderBody created with ${placeOrderBody.cart?.length ?? 0} cart items');
    ...
    if (cartList != null) {
      for (var cartModel in cartList) {
        if (Get.find<SplashController>()
            ...
            .newVariation!) {
          double itemPrice = cartModel.item!.price!;
          ...
          price = price + (itemPrice * cartModel.quantity!);
        } else {
          // For items without new variations, calculate price for this specific item
          double itemPrice = cartModel.item!.price!;
          if (B2BPricingHelper.shouldUseB2BPricing()) {
            final b2bPrice =
                B2BPricingHelper.getB2BContractPrice(cartModel.item!.id!);
          ...
          if (cartModel.item!.variations!.isNotEmpty &&
            ...
            String variationType = cartModel.variation![0].type!;
            for (Variation variation in cartModel.item!.variations!) {
              if (variation.type == variationType) {
                ...
                price = price + (variationPrice * cartModel.quantity!);
                break;
              }
            }
          } else {
            // No variations, use base item price
            price = price + (itemPrice * cartModel.quantity!);
          }
        }
      }
    ...
    if (store != null && cartList != null) {
      for (var cartModel in cartList) {
        ...
        for (int index = 0; index < addOnList.length; index++) {
          ...
          addOns = addOns + (addonPrice * cartModel.addOnIds![index].quantity!);
        }
      }
    }
      ...
      for (var cartModel in cartList) {
        ...
        if (Get.find<SplashController>()
          ...
          for (int index = 0;
            ...
            for (int i = 0;
              ...
              if (cartModel.foodVariations![index][i]!) {
                ...
                double variationOptionPrice = cartModel.item!
          ...
          if (cartModel.item!.variations!.isNotEmpty) {
            for (Variation variation in cartModel.item!.variations!) {
              if (variation.type == variationType) {
                // Apply B2B pricing to regular variations
                double variationOptionPrice = variation.price!;
                if (B2BPricingHelper.shouldUseB2BPricing()) {
                  final b2bVariationPrice =
                      B2BPricingHelper.getB2BContractVariationPrice(
                          cartModel.item!.id!, variationType);
                ...
                variationPrice += (variationOptionPrice * cartModel.quantity!);
                break;
              }
            }
          } else {
            // Apply B2B pricing to item price when no variations
            double itemPrice = cartModel.item!.price!;
            if (B2BPricingHelper.shouldUseB2BPricing()) {
              final b2bPrice =
                  B2BPricingHelper.getB2BContractPrice(cartModel.item!.id!);
            ...
            variationDiscount += (PriceConverter.convertWithDiscount(
                    itemPrice, discount, discountType)! *
                cartModel.quantity!);
            variationPrice += (itemPrice * cartModel.quantity!);
          }
        }
      }
    }
    ...
    if (store != null && cartList != null) {
      for (var cartModel in cartList) {
        ...
        if (Get.find<SplashController>()
            .getModuleConfig(cartModel.item!.moduleType)
            .newVariation!) {
          // Apply B2B pricing to base item price for discount calculation
          double itemPrice = cartModel.item!.price!;
          if (B2BPricingHelper.shouldUseB2BPricing()) {
            final b2bPrice =
                B2BPricingHelper.getB2BContractPrice(cartModel.item!.id!);
          ...
          if (cartModel.item!.variations!.isNotEmpty) {
            for (Variation variation in cartModel.item!.variations!) {
              if (variation.type == variationType) {
                // Apply B2B pricing to variation price for discount calculation
                double variationOptionPrice = variation.price!;
                if (B2BPricingHelper.shouldUseB2BPricing()) {
                  final b2bVariationPrice =
                      B2BPricingHelper.getB2BContractVariationPrice(
                ...
                variationPrice += (PriceConverter.convertWithDiscount(
                        variationOptionPrice, dis, disType)! *
                    cartModel.quantity!);
                variationWithoutDiscountPrice +=
                    (variationOptionPrice * cartModel.quantity!);
                break;
              }
            }
            discount =
    ...
    if (cartModel != null) {
      ...
      for (int index = 0;
        ...
        for (int i = 0;
            ...
            i++) {
          if (cartModel.foodVariations![index][i]!) {
            // Apply B2B pricing to food variation prices
            ...
            variationPrice += (PriceConverter.convertWithDiscount(
                    ...
                    isFoodVariation: true)! *
                cartModel.quantity!);
            variationDiscount += (variationOptionPrice * cartModel.quantity!);
          }
        }
      }
    }
  ...
  bool _checkPrescriptionRequired() {
    if (widget.storeId == null &&
      ...
      for (var cart in currentCartList) {
