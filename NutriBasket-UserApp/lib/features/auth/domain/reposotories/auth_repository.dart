...
/*  @override
  ...
  Future<Response> updateToken({String notificationDeviceToken = ''}) async {
    ...
    final String? tokenToSend = notificationDeviceToken.isNotEmpty
        ? notificationDeviceToken
        : deviceToken;

    // Short-circuit if no valid token available to avoid clearing existing token on transient failures
    if (tokenToSend == null || tokenToSend == '@' || tokenToSend.isEmpty) {
      if (kDebugMode) {
        print(
            '--------No valid FCM token available, skipping backend update to preserve existing token----------');
      }
      return Response(statusCode: 200, statusText: 'No token to update');
    }

    if (kDebugMode) {
      print('--------Sending FCM Token to Backend---------- $tokenToSend');
    }
    return await apiClient.postData(AppConstants.tokenUri,
        {"_method": "put", "cm_firebase_token": tokenToSend},
        handleError: false);
  }
  ...
  Future<String?> saveDeviceToken() async {
    ...
    if (!GetPlatform.isWeb) {
      ...
      deviceToken = '@'; // Web platform marker
    }
    return deviceToken;
  }
