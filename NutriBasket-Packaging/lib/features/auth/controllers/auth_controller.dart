...
class AuthController extends GetxController implements GetxService {
  ...
  Future<ResponseModel> login(String phone, String password) async {
    ...
    if (response.statusCode == 200) {
      ...
      // await authServiceInterface.updateToken(); // Skipped FCM update for now
      responseModel = ResponseModel(true, 'successful');
    } else {
      responseModel = ResponseModel(
          false, response.statusText ?? 'An unknown error occurred');
    }
    _isLoading = false;
    update();
  ...
  void saveUserNumberAndPassword(String number, String password,
      String countryDialCode, String countryCode) {
    authServiceInterface.saveUserNumberAndPassword(
        number, password, countryDialCode, countryCode);
  }
