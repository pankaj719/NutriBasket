...
class OrderController extends GetxController implements GetxService {
  ...
  // New Packager Order State
  List<OrderModel>? _assignedOrderList;
  List<OrderModel>? get assignedOrderList => _assignedOrderList;

  List<OrderModel>? _statusOrderList;
  List<OrderModel>? get statusOrderList => _statusOrderList;

  List<OrderModel>? _packagingOrderList;
  List<OrderModel>? get packagingOrderList => _packagingOrderList;

  List<OrderModel>? _pickedUpOrderList;
  List<OrderModel>? get pickedUpOrderList => _pickedUpOrderList;

  OrderModel? _packagerOrderDetails;
  OrderModel? get packagerOrderDetails => _packagerOrderDetails;
  ...
  // New Packager Methods
  Future<void> getAssignedOrders() async {
    _assignedOrderList = await orderServiceInterface.getAssignedOrders();
    update();
  }

  Future<void> getOrdersByStatus(String status) async {
    _statusOrderList = await orderServiceInterface.getOrdersByStatus(status);
    update();
  }

  Future<void> getPackagingOrders() async {
    _packagingOrderList = await orderServiceInterface.getPackagingOrders();
    update();
  }

  Future<void> getPickedUpOrders() async {
    _pickedUpOrderList = await orderServiceInterface.getPickedUpOrders();
    update();
  }

  Future<void> getPackagerOrderDetails(int orderId) async {
    _packagerOrderDetails = null;
    Response response =
        await orderServiceInterface.getPackagerOrderDetails(orderId);
    if (response.statusCode == 200) {
      _packagerOrderDetails = OrderModel.fromJson(response.body);
    }
    update();
  }

  Future<bool> updatePackagerOrderStatus(Map<String, dynamic> body) async {
    _isLoading = true;
    update();
    ResponseModel responseModel =
        await orderServiceInterface.updatePackagerOrderStatus(body);
    _isLoading = false;
    update();
    if (responseModel.isSuccess) {
      showCustomSnackBar(responseModel.message, isError: false);
