class ProfileModel {
  ...
  ProfileModel.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    // Handle both delivery-man and packager response formats
    fName = json['f_name'] ?? json['name']?.split(' ').first;
    lName = json['l_name'] ?? json['name']?.split(' ').skip(1).join(' ');
    phone = json['phone'];
    email = json['email'];
    identityNumber = json['identity_number'];
    identityType = json['identity_type'];
    // Safe casting for identity_image_full_url
    ...
    zoneId = json['zone_id'];
    active = json['active'] ?? 1; // Default to active for packagers
    avgRating = json['avg_rating']?.toDouble();
    ratingCount = json['rating_count'];
    memberSinceDays =
        json['member_since_days'] ?? 0; // Default to 0 for packagers
    orderCount = json['order_count'] ?? 0; // Default to 0 for packagers
    todaysOrderCount =
        json['todays_order_count'] ?? 0; // Default to 0 for packagers
    thisWeekOrderCount =
        json['this_week_order_count'] ?? 0; // Default to 0 for packagers
    cashInHands =
        json['cash_in_hands']?.toDouble() ?? 0.0; // Default to 0 for packagers
    earnings = json['earning'] ??
        0; // Default to 0 for packagers (no earnings for packagers)
    type = json['type'] ?? 'packager'; // Default to packager type
    balance = json['balance']?.toDouble() ?? 0.0; // Default to 0 for packagers
