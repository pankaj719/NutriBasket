...
class ReviewReplyScreen extends StatefulWidget {
  ...
  const ReviewReplyScreen(
      {super.key,
      required this.isGiveReply,
      required this.review,
      this.storeReviewReplyStatus = false});
    ...
    return Scaffold(
      appBar: CustomAppBarWidget(
        titleWidget: Column(children: [
          Text(
              !widget.storeReviewReplyStatus!
                  ? 'review'.tr
                  : widget.storeReviewReplyStatus! && widget.isGiveReply
                      ? 'review_reply'.tr
                      : 'update_reply'.tr,
                      ...
                      widget.storeReviewReplyStatus!
                          ? widget.isGiveReply
                              ...
                              : Container(
                                  ...
                                  child: Text(widget.review.reply ?? '',
                                      style: robotoRegular,
                                      maxLines: 5,
                                      overflow: TextOverflow.ellipsis),
                                )
              ...
              ? Container(
                  ...
                  child: !storeController.isLoading
                      ? CustomButtonWidget(
                          onPressed: () {
                            if (widget.isGiveReply) {
                              storeController.updateReply(
                                  widget.review.id!, _replyController.text);
                            } else {
                              Get.offNamed(RouteHelper.getReviewReplyRoute(
                                  isGiveReply: true,
                                  review: widget.review,
                                  storeReviewReplyStatus:
                                      Get.find<SplashController>()
                                          .configModel!
                                          .storeReviewReply!));
                            }
                          },
                          buttonText: widget.isGiveReply
                              ? 'send_reply'.tr
                              : 'update_review'.tr,
