...
class _UpdateStockBottomSheetState extends State<UpdateStockBottomSheet> {
  ...
  void initState() {
    ...
    variationStockControllers = widget.item.variations!
        .map((variation) => TextEditingController(
              text: variation.stock.toString(),
            ))
        .toList();
    priceControllers = widget.item.variations!
        .map((variation) => TextEditingController(
  ...
  void _setTotalStock() {
    ...
    for (TextEditingController stockController in variationStockControllers) {
      totalStock = stockController.text.trim().isNotEmpty
  ...
  Widget build(BuildContext context) {
    return Container(
      width: context.width,
      constraints:
          BoxConstraints(maxHeight: context.height * 0.8, minHeight: 0.3),
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(
            height: variationStockControllers.isNotEmpty
                ? Dimensions.paddingSizeDefault
                : Dimensions.paddingSizeExtraLarge),
        variationStockControllers.isNotEmpty
            ? Flexible(
                        ...
                        children: [
                          Row(children: [
                            Expanded(
                                flex: 3,
                                child:
                                    Text('variation'.tr, style: robotoMedium)),
                            Expanded(
                                flex: 3,
                                child: Text('price'.tr, style: robotoMedium)),
                            const SizedBox(width: Dimensions.paddingSizeSmall),
                            Expanded(
                                flex: 3,
                                child: Text('stock'.tr, style: robotoMedium)),
                          ]),
                          const SizedBox(height: Dimensions.paddingSizeDefault),
                          ListView.builder(
              ...
              child: GetBuilder<StoreController>(builder: (storeController) {
                return !storeController.isLoading
                    ? CustomButtonWidget(
                        ...
                        onPressed: () {
                          if (mainStockController.text == '0' ||
                              variationStockControllers
                                  .any((element) => element.text == '0')) {
  ...
  void _updateStock(StoreController storeController) {
    ...
    for (var variation in widget.item.variations!) {
      data.addAll({
        "price_${widget.item.variations!.indexOf(variation)}_${variation.type}":
            priceControllers[widget.item.variations!.indexOf(variation)]
                .text
                .trim()
      });
    }
    ...
    for (var variation in widget.item.variations!) {
      data.addAll({
        "stock_${widget.item.variations!.indexOf(variation)}_${variation.type}":
            variationStockControllers[
                    widget.item.variations!.indexOf(variation)]
                .text
                .trim()
      });
    }
