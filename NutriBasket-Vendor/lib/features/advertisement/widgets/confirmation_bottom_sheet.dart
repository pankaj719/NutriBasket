      ...
      child: GetBuilder<AdvertisementController>(
        ...
        return SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            widget.status == 'cancel_ads' || widget.status == 'pause_ads'
                ? Form(
                    key: advertisementController.noteFormKey,
                    child: CustomTextFieldWidget(
                      inputType: TextInputType.text,
                      controller: advertisementController.noteController,
                      hintText: widget.status == 'cancel_ads'
                          ? "cancelation_note".tr.replaceAll(":", "")
                          : 'pause_note'.tr,
                      ...
                      validator: (value) {
                        return (value == null || value.isEmpty)
                            ? widget.status == 'cancel_ads'
                                ? 'enter_cancellation_note'.tr
                                : 'enter_paused_note'.tr
                            : null;
                      },
                    ),
                  )
            ...
            Row(children: [
              widget.isShowNotNowButton
                  ...
                  : const SizedBox(),
              SizedBox(
                  width: widget.isShowNotNowButton
                      ? Dimensions.paddingSizeDefault
                      : 0),
              Expanded(
                  flex: 2,
                  child: advertisementController.isLoading
                      ? const Center(child: CircularProgressIndicator())
                      : CustomButtonWidget(
                          buttonText: widget.confirmButtonText?.tr ?? "yes".tr,
                          onPressed: !advertisementController.isLoading
                              ? widget.yesButtonPressed
                              : () {},
                          color: widget.yesTestColor ??
